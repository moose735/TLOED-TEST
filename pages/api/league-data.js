...
async function fetchLeagueHistory(currentLeagueId, baseUrl, history = []) {
  ...
  try {
    ...
    // For a simple solution, we'll look for the roster with the highest `fpts` in the last week,
    // or just assume the 'rank 1' roster from the final standings.

    // Let's refine champion finding: The `previous_league_id` links to the *previous* season's league.
    // The current league object's `settings.season` is the season this league *represents*.
    // We can infer the champion by finding the roster with the most wins in the *current* league object.
    // Or, more accurately, the roster that *won* the championship game. The API doesn't directly give this.
    // A common proxy: the team with rank 1 in the final standings (which requires fetching matchups for the last week).
    // For now, let's simplify and just grab the highest scoring team as a placeholder if no explicit champion field exists.
